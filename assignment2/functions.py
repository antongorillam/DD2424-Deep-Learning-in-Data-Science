...
def plotLostVEpochs(train, test, metric, x_axis, title_string, dir, y_lim=[0,2]):
	import matplotlib.pyplot as plt

	[y_min, y_max] = y_lim
	xs = np.linspace(1, 1000, len(train))
	plt.figure()
	plt.plot(xs, train, label=f'Training {metric}', color='g')
	if test != []:
		plt.plot(xs, test, label=f'Validation {metric}', color='r')
